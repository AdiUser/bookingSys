...
<body class='default' style="height:100%; margin-top:-30px;">
                ...
                <form name="stdwelcome" action="stdwelcome.php" method="post">
                    <ul id="menu">
                     <?php if(isset($_SESSION['stdname'])){ ?>
                        ...
                        <li><a href="stdwelcome.php"><input type="button" value="Home" name="action" class="subbtn" /></a></li>
    ...
    <div class="page" style="width:100%;">
		...
		<div id="performance">
						<?php
			$tq=$w=$c=$s=$a=0;
				$query=executeQuery("select testid,testname,totalquestions from test where testid in (select distinct(testid) from studenttest where stdid=$sid order by starttime)");
				if($query)
				{
				echo "<table class='datatable'>
					...
					{	
						...
						$attempted=0;
						$wrong=0;						
						$count_query=executeQuery("SELECT * FROM `studentquestion` where stdid=$sid and testid=$resu[0]");
						if($count_query)
						{
							...
							$score_query=executeQuery("SELECT score FROM `studenttest` where stdid=$sid and testid=$resu[0]");
