...
$marks_query="select testname from test where testid=$_REQUEST[testid]";
...
<html>
    <head>
        ...
        <title>OES: <?php echo "$row8[0]";?> Statistics</title>
...
<script type="text/javascript">
        $(document).ready(function () {
            ...
            <?php
                ...
                $f=0;
				$cnt=0;
                $r=executeQuery("SELECT st.stdid,st.stdname FROM studenttest as s INNER JOIN student as st on s.stdid=st.stdid WHERE s.testid=$_REQUEST[testid] and s.status='over' order by st.stdname");
                while ($row=  mysql_fetch_array($r))
                {
					$cnt=$cnt+1;
                    $tes=$tms=$ths=0;
                    //$r2=executeQuery("select * from level where testid=$_REQUEST[testid]");
                    //$row2=mysql_fetch_array($r2);
                    //-----Easy Questions---------
                    $r3=executeQuery("select q.testid,q.qnid from subj_question as s inner join question as q on s.qnid_s=q.qnid_s where s.level='level_e' and q.testid=$_REQUEST[testid]");
					while($row3=  mysql_fetch_array($r3))
                    {
                        $r4=executeQuery("select marks from studentquestion where testid=$_REQUEST[testid] and qnid=$row3[1] and stdid=$row[0]");
                    ...
                    //------Medium Questions-------
                    $r3=executeQuery("select q.testid,q.qnid from subj_question as s inner join question as q on s.qnid_s=q.qnid_s where s.level='level_m' and q.testid=$_REQUEST[testid]");
                    while($row3=  mysql_fetch_array($r3))
                    {
                        $r4=executeQuery("select marks from studentquestion where testid=$_REQUEST[testid] and qnid=$row3[1] and stdid=$row[0]");
                        if(mysql_num_rows($r4) > 0)
                    ...
                    //----------Hard Questions-----------
                    $r3=executeQuery("select q.testid,q.qnid from subj_question as s inner join question as q on s.qnid_s=q.qnid_s where s.level='level_h' and q.testid=$_REQUEST[testid]");
                    while($row3=  mysql_fetch_array($r3))
                    {
                        $r4=executeQuery("select marks from studentquestion where testid=$_REQUEST[testid] and qnid=$row3[1] and stdid=$row[0]");
                        if(mysql_num_rows($r4) > 0)
                    ...
                    $r5=executeQuery("select total from test where testid=$_REQUEST[testid]");
                    ...
                    if($f == 0)
                    {
                        echo "{ Name: '$row[1]', Easy: $a1, Medium: $a2, Hard: $a3, Total: $tot }";
                    }
                    else
                    {
                        echo ",{ Name: '$row[1]', Easy: $a1, Medium: $a2, Hard: $a3, Total: $tot }";
            ...
            var settings = {
                ...
                description: "Score in <?php echo $row8[0] ?>",
...
<body style="height:100%; margin-top:-30px;">
    <div id="container">
      <div class="header">
    ...
    <div class="page" style="width:100%;">
		...
		<div style="width:'100%'; overflow:auto;">    
			<div id='jqxChart' style="<?php if($cnt <= 20) echo "width: 98%;"; else if($cnt > 30 && $cnt <=50) echo "width: 5000px;"; else echo "width: 8000px;"; ?> height: 400px;text-align: center;padding:15px;"></div>
		</div>
